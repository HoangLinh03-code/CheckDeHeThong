...
def simplify_math_text(text):
    ...
    # FIX: Giữ lại nội dung điều kiện phản ứng (t^o, xt) trên và dưới mũi tên thay vì xóa sạch
...
def check_formula_issues(pdf_content, sys_content, sys_opts):
    ...
    if long_formulas:
        ...
        for formula in long_formulas:
            ...
            # Trích xuất các điều kiện đặc biệt trên/dưới mũi tên (ví dụ t^o, xt) để check chéo
            conditions = []
            for m in re.finditer(r'\\xrightarrow(?:\[(.*?)\])?\{(.*?)\}', formula):
                cond1 = simplify_math_text(m.group(1) or "")
                cond2 = simplify_math_text(m.group(2) or "")
                # Thêm dấu '=' vào trước để chắc chắn nó nằm đúng vị trí mũi tên
                if cond1 or cond2:
            ...
            if len(sys_simple) < L:
                if SequenceMatcher(None, form_simple, sys_simple).ratio() >= threshold:
                    # Bắt buộc phải chứa điều kiện phản ứng (nếu có)
                    if all(c in sys_simple for c in conditions):
                        found = True
            else:
                # Duyệt các cửa sổ với độ dài linh hoạt từ L đến L+3
                    ...
                    for delta in range(0, 4): 
                        if i + L + delta <= len(sys_simple):
                            ...
                            if SequenceMatcher(None, form_simple, window).ratio() >= threshold:
                                # Bắt buộc phải chứa điều kiện phản ứng trong chính đoạn vừa khớp
                                if all(c in window for c in conditions):
                                    found = True
                                    break
                    if found:
                        break
...
if __name__ == "__main__":
    ...
    if len(_sys.argv) == 3:
        ...
        PDF_FILE = "D:\CheckTool\FetchJson\dapan_Hoa_pdf.json"
        SYS_FILE = "D:\CheckTool\FetchJson\ĐỀ hóa 10_ APP ÔN LUYỆN 1_69d4b8a16c5530c776f02e08_sys.json"
